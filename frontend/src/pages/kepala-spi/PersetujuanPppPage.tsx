...
import type { Ppp } from '../../services/kepalaSpiService'
import { approvePpp, getPppList, kembalikanPpp } from '../../services/kepalaSpiService'
...
/**
 * Tahap 07 flowmap SIGMA v3.0 -- Kepala SPI (Penanggung Jawab) memberi
 * persetujuan akhir atas PPP yang sudah diteruskan Pengawas. Setelah
 * disetujui, Dukungan Audit bisa membuat draf Surat Tugas-nya (tahap 08).
 */
export function PersetujuanPppPage() {
  ...
  const menungguCount = pppList.filter((p) => p.status === 'Diteruskan').length
  const dalamTelaahCount = pppList.filter((p) => p.status === 'Diajukan').length
    ...
    >
      <div>
        ...
        <p className="mt-1 text-sm text-slate-500">
          Setujui Program &amp; Pengajuan Penugasan yang telah ditelaah dan diteruskan Pengawas.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        ...
        />
        <StatCard label="Masih di Pengawas" value={dalamTelaahCount} icon={Send} subtitle="Belum diteruskan" />
                    ...
                    {p.status === 'Diteruskan' && (
                      <>
                        ...
                        <button
                          type="button"
                          disabled={isBusy}
                          onClick={() => void runAction(p.pppId, () => approvePpp(p.pppId), 'Gagal menyetujui PPP.')}
