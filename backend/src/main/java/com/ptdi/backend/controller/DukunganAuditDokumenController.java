...
/**
 * Dukungan Audit (role tunggal, staf & koordinator sudah digabung) menyusun
 * draf "Dokumen Program" (DOK PROG di diagram alur) dan mengajukannya
 * sendiri ke Kepala SPI (status Draft -> Diajukan) lewat endpoint ajukan()
 * di bawah. Pemeriksaan (Checked) & persetujuan (Approved) dokumen yang
 * sudah diajukan dilakukan Kepala SPI, lihat KepalaSpiDokumenController.
 */
@RestController
...
public class DukunganAuditDokumenController {
    ...
    /** Berkas PDF bisa dilihat siapa saja yang punya akses ke Dokumen Program ini (Dukungan Audit & Kepala SPI). */
    @GetMapping("/{id}/file")
